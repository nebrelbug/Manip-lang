...
function requestFile(path) {
    ...
    //generate parameter can either be true or false depending on whether to generate an Ast for the file
      ...
      async function storeContents() {
        if (httpRequest.readyState === XMLHttpRequest.DONE) {
          if (httpRequest.status === 200) {
            retrievedFile = await httpRequest.responseText;
            console.log("response text: " + httpRequest.responseText);
            testFile = await httpRequest.responseText;
            var expressions = getExpressions(testFile, false)
            console.log("expressions at 230: " + expressions)
            console.log("final ast: " + JSON.stringify(generateAst(expressions)))
          } else {
    ...
    function getExpressions(file, miniAst) {
        //var noWhiteSpaceFile = file.toString().replace(/\s/g, '');
        var expressions;
        if (miniAst) {
...
function generateAst(expressions) {
  ...
  var testFile = requestFile("test.mnip")
